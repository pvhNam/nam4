...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupRecyclerViews() {
        ...
        rvFeaturedSales.setLayoutManager(
                new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false));
        rvRentalCars.setLayoutManager(
                new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false));
        ...
        carRentalAdapter = new CarRentalAdapter(rentalCarList, car -> {
            Intent intent = new Intent(MainActivity.this, CarDetailActivity.class);
            intent.putExtra("CAR_DATA", car);
            startActivity(intent);
        });
        ...
        db.collection("cars").get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    ...
                    for (QueryDocumentSnapshot doc : queryDocumentSnapshots) {
                        ...
                        car.setCarId(doc.getId());
                        car.setUserId(uid);

                        boolean isSale = normalizedType.equals("sale")
                                || normalizedType.contains("bán")
                                || normalizedType.contains("ban");
                        boolean isRental = normalizedType.equals("rental")
                                || normalizedType.contains("thuê")
                                || normalizedType.contains("thue");

                        if (isSale)   saleCarList.add(car);
                        else if (isRental) rentalCarList.add(car);
                    }
    ...
    private void loadDummyData() {
        saleCarList.add(new Car("Toyota Vios 2020", "450.000.000 VNĐ",
                "TP.HCM • 2020 • Tự động", android.R.drawable.ic_menu_gallery));
        saleCarList.add(new Car("Kia Morning 2021", "320.000.000 VNĐ",
                "Hà Nội • 2021 • Số sàn", android.R.drawable.ic_menu_gallery));
        rentalCarList.add(new Car("Hyundai Accent 2022", "800.000đ / ngày",
                "Quận 1, TP.HCM • Tự động", android.R.drawable.ic_menu_gallery));
        carSaleAdapter.filterList(saleCarList);
        carRentalAdapter.filterList(rentalCarList);
    }
    ...
    private void setupAutoComplete() {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<>(this,
                android.R.layout.simple_dropdown_item_1line, carNames);
