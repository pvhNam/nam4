...
import com.example.doanmb.databinding.ActivityFullscreenMediaBinding;
...
/** Xem ảnh/video toàn màn hình (vuốt ngang). Màn thuần hiển thị nên không cần ViewModel. */
public class FullscreenMediaActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.btnCloseMedia.setOnClickListener(v -> finish());
        binding.viewpagerMedia.setAdapter(new MediaPagerAdapter());
        binding.viewpagerMedia.setCurrentItem(start, false);
        updateCounter(start);

        binding.viewpagerMedia.registerOnPageChangeCallback(new ViewPager2.OnPageChangeCallback() {
    ...
    private void updateCounter(int pos) {
        if (urls.size() <= 1) {
            binding.tvMediaCounter.setVisibility(View.GONE);
        } else {
            binding.tvMediaCounter.setVisibility(View.VISIBLE);
            binding.tvMediaCounter.setText((pos + 1) + " / " + urls.size());
        }
    }

    private void releaseInactivePlayers(int current) {
        for (Map.Entry<Integer, ExoPlayer> entry : activePlayers.entrySet()) {
            if (entry.getKey() != current && entry.getValue() != null) entry.getValue().pause();
        }
    }
    ...
    private class MediaPagerAdapter extends androidx.recyclerview.widget.RecyclerView.Adapter<MediaPagerAdapter.VH> {
        ...
        public void onBindViewHolder(@NonNull VH holder, int position) {
            ...
            if (isVideo) setupVideo(holder, position, url); else setupImage(holder, url);
        }
        ...
        private void setupVideo(VH h, int position, String url) {
            ...
            player.setMediaItem(MediaItem.fromUri(CloudinaryHelper.getOptimizedVideoUrl(url)));
            player.prepare();
            player.setPlayWhenReady(true);
            player.addListener(new Player.Listener() {
                @Override public void onPlaybackStateChanged(int state) {
                    h.progressBar.setVisibility(state == Player.STATE_BUFFERING ? View.VISIBLE : View.GONE);
                }
        ...
        private void setupImage(VH h, String url) {
            ...
            String optimized = url.contains("cloudinary.com")
                    ? url.replaceAll("/upload/[^/]+/", "/upload/") : url;
    ...
    protected void onPause() {
        super.onPause();
        for (ExoPlayer player : activePlayers.values()) if (player != null) player.pause();
    }

    @Override
    protected void onDestroy() {
        super.onDestroy();
        for (ExoPlayer player : activePlayers.values()) if (player != null) player.release();
        activePlayers.clear();
    }
}
